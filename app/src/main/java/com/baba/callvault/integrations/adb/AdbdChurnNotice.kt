 ...
 * Shizuku server says so instead of leaving it a mystery.
 *
 ...
 *
 * See [ShizukuChurnPolicy] for why this only ever *tells* and never cancels.
 */
object AdbdChurnNotice {
    ...
    /**
     * Runs [block], which restarts `adbd`, and warns the user if that killed a running Shizuku server.
     *
     ...
     * Shizuku "disabling automatically".
     *
     * @param what a short description of the operation, for the log.
     * @param underDialog true when a dialog on screen already warned the user; it is not repeated.
     */
    fun <T> around(context: Context, what: String, underDialog: Boolean = false, block: () -> T): T {
        ...
        when (advice) {
            ...
            ShizukuChurnAdvice.WARN_FIRST ->
                AppLogger.i(TAG, "$what restarted adbd and stopped Shizuku; the user was warned first")
            ShizukuChurnAdvice.TELL_AFTER -> {
                AppLogger.w(TAG, "$what restarted adbd, which stopped the Shizuku server that was running")
                runCatching { SilentFailureNotifier.warnShizukuStopped(context) }
                    .onFailure { AppLogger.w(TAG, "Could not post the Shizuku notice: ${it.message}") }
            }
        }
        return result
    }
